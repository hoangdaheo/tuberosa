...
function createServices() {
  ...
  const models = new HashModelProvider(CALIBRATION_CONFIG.model.embeddingDimensions);
...
function newSourceMap(): Record<CandidateSource, number> {
  return { metadata: 0, lexical: 0, vector: 0, memory: 0, graph: 0, worktree: 0, atoms: 0, userStyle: 0, convention: 0 };
}
