...
**Known bug (surfaced 2026-05-22 while starting Phase 6, FIXED 2026-05-22 in the audit pass):**
...
- **Fix landed** in the post-Phase-10 audit pass: `src/storage/postgres-store.ts` now defines a module-local `isPersistedKnowledgeId(value)` (UUID-shape regex) + `filterPersistedKnowledgeIds(ids)` helper and applies them at every method that takes a knowledge id from outside and casts to `::uuid` / `::uuid[]`: `getKnowledge` (single id, returns `undefined`), `listKnowledgeRelations` (`fromKnowledgeId` / `targetKnowledgeId`, returns `[]`), `listKnowledgeChunks` (`knowledgeIds[]`), `searchLexical` / `searchVector` / `searchMetadata` / `searchMemories` / `searchGraphRelations` (`rejectedKnowledgeIds` + `seedKnowledgeIds`), `getFeedbackSummaries` (`knowledgeIds[]`), `recordFeedback` (`rejectedKnowledgeIds[]`, also feeds `expireRelationsFromKnowledge`), `recordAgentContextDecision` (`rejectedKnowledgeIds[]`), `listLearningProposals` (`affectedKnowledgeId`). The `MemoryKnowledgeStore` was already permissive — this brings Postgres in line so synthetic worktree ids are silently dropped instead of crashing the uuid cast. Behavior is consistent: worktree candidates are recomputed fresh per query, so dropping their ids from persisted-feedback inserts is correct (the worktree provider can't durably reject by id).
- **Regression test:** `test/integration.test.ts` gained "Postgres store silently filters Phase-5 worktree synthetic ids from uuid casts" — ingests a real knowledge item, then asserts that `getFeedbackSummaries`, `listKnowledgeChunks`, `getKnowledge`, `searchLexical(rejectedKnowledgeIds:[synth])`, and `recordFeedback(rejectedKnowledgeIds:[synth])` all succeed with mixed and pure-synthetic id inputs. Docker-gated; skips when Postgres isn't reachable.
- **Verification (all green):** `pnpm run build` ✅, `pnpm test` ✅ 284/284 (was 283; +1 from the new integration test), `pnpm run eval:retrieval` ✅ 14/14 / context-fit 100%, `pnpm run eval:agent-context` ✅, `pnpm run eval:context-mapping` ✅ — precision 25%, recall 100%, entities 100%, noise 75%, placement 100%, fit 100%, forbidden 0%, brief 100% (no regressions vs Phase 10), `pnpm run eval:safety` ✅ 100/100/100, `pnpm run sandbox` ✅ hit 93.2%, MRR 0.4771, noise 9.1%, p50 22ms, p95 43ms (PASS thresholds). The four-test Docker-gated integration suite now passes against the live `tuberosa-postgres-1` container (3.5s round-trip with seed + verify).
- **Fallout / carry-overs:** the docker-compose `tuberosa-app-1` and `tuberosa-worker-1` containers are still on the pre-fix build (started 10 min before this patch); they pick up the fix on the next `docker compose up --build`. The MemoryKnowledgeStore is unchanged — it never crashed and the new helpers live only in `postgres-store.ts`. No new env flag was added, consistent with Phases 1-10. If a future synthetic id namespace lands (e.g. `live-edit:<sha>`), revisit the UUID regex — broaden to "drop any id failing the UUID shape" rather than enumerating prefixes (which the current helper already does).
...
- **Phase 5 Postgres UUID crash on worktree ids (RESOLVED 2026-05-22 in the post-Phase-10 audit pass).** Was still open at Phase 9 write-time; the audit pass landed `isPersistedKnowledgeId` + `filterPersistedKnowledgeIds` helpers in `src/storage/postgres-store.ts` at every uuid-cast boundary and added a Docker-gated regression test in `test/integration.test.ts`. See the updated "Known bug" block under Phase 5 above for the full fix details.
...
- **Postgres UUID crash on worktree ids (Phase 5 carry-over) — RESOLVED 2026-05-22 in the post-Phase-10 audit pass.** Was still open at Phase 10 write-time; the audit pass landed the filter helpers in `src/storage/postgres-store.ts` and added a Docker-gated regression test. See the updated "Known bug" block under Phase 5 for the full fix.
...
## Audit pass — 2026-05-22 (post-Phase-10)

**Trigger:** "Audit each phase, check the bug, resolve it if needed" — a sweep across the eleven phase status blocks against the current code.

**Findings:**
- **Phases 0-10 are all implemented and green** at the time of the audit. The verification suite reproduces the per-phase numbers: `pnpm run build` ✅, `pnpm test` ✅ 283/283 pre-audit (284/284 with the new integration test added in this pass), `pnpm run eval:retrieval` ✅ 14/14 with context-fit 100%, `pnpm run eval:agent-context` ✅, `pnpm run eval:context-mapping` ✅ precision 25% / recall 100% / entities 100% / noise 75% / placement 100% / fit 100% / forbidden 0% / brief 100%, `pnpm run eval:safety` ✅ 100/100/100 across all five secret patterns, `pnpm run sandbox` ✅ hit 93.2% / MRR 0.4771 / noise 9.1% / p50 22ms / p95 43ms.
- **One open bug, fixed:** the Phase 5 / Phase 9 / Phase 10 carry-over — `worktree:<sha256>` ids crashing the Postgres `::uuid` casts — reproduced on the very first `tuberosa_start_session` call of the audit (same error string the plan documented at line ~516). Patched as described in the updated "Known bug (RESOLVED 2026-05-22)" block under Phase 5; the Phase 9 and Phase 10 carry-over lines were updated to point at it.
- **No other carry-overs surfaced new failures.** Every remaining carry-over in each phase's "Tried but not done" list is still applicable as written (synthetic-stream churn metric, worktree-precedence fixture, OpenAI-side latency measurement of the rewrite gate, fixture demonstrating brief-groundedness drops, persisted `MaintenanceBatch` across MCP restarts, etc.). None block correctness.

**Code changes this pass:**
- `src/storage/postgres-store.ts` — module-local `UUID_PATTERN` regex + `isPersistedKnowledgeId(value)` + `filterPersistedKnowledgeIds(ids)` helpers; filter applied at the 11 uuid-cast boundary methods enumerated under the Phase 5 fix block.
- `test/integration.test.ts` — new Docker-gated regression test (`Postgres store silently filters Phase-5 worktree synthetic ids from uuid casts`) seeded with a real knowledge id and a pair of synthetic `worktree:<sha>` ids; exercises `getFeedbackSummaries`, `listKnowledgeChunks`, `getKnowledge`, `searchLexical(rejectedKnowledgeIds:[synth])`, and `recordFeedback(rejectedKnowledgeIds:[synth])`.
- `implements/enhance_rewrite/tuberosa-enhance-knowledge-quality.md` — this audit block + the Phase 5 "Known bug" status flip from "NOT FIXED YET" to "FIXED 2026-05-22" + the Phase 9/10 carry-over pointers.

**Tried but not done in this pass (deliberate carry-overs):**
- **Docker stack rebuild.** The fix is in the source tree but `tuberosa-app-1` / `tuberosa-worker-1` are still on the pre-fix build (started ~10 min before the patch). They'll pick up the fix on the next `docker compose up --build`. The Docker-gated integration test runs against the same `tuberosa-postgres-1` instance and confirms the store-side fix; the app containers are merely wrapping the store via HTTP and inherit the corrected behavior the next time their JS bundle is rebuilt.
- **Live MCP smoke against the running stack.** Did not re-run `tuberosa_start_session` end-to-end against the docker app server (which still runs the pre-fix bundle). The integration test reproduces every uuid-cast call site the MCP path can hit.
- **Generalize the synthetic-id filter to a `KnowledgeStore`-level contract.** Today the predicate lives in `postgres-store.ts` only. If a third store implementation lands that also has UUID-shaped persistence, copy the helper there. The `MemoryKnowledgeStore` was already permissive so it doesn't need it.
